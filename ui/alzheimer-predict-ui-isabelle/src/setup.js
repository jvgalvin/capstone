...
(async () => {
    ...
    for (const patient of patient_data) {
        ...
        values = [`${patient_id.rows[0]["id"]}`, "LMCI", `${patient["APOE4"]}`,`${patient["MMSE"]}`,`${patient["Age"]}`,`${patient["Gender"]}`,
                  `${patient["Years_of_Education"]}`, `${patient["Ethnicity"]}`, `${patient["Race"]}`]
        columns_to_not_use = ["patient_id", "Diagnosis_at_Baseline", "APOE4", "MMSE", "Age", "Gender", "Years_of_Education", "Ethnicity", "Race"]
        for (const d of Object.keys(data)) {
